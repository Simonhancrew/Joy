class Solution {
 public:
  ...
  void split(string const &ip, char tar) {
    ...
    for (int i = 0; i < n; i++) {
      int j = i;
      while (j < n && ip[j] != tar) j++;
      rec.push_back(ip.substr(i, j - i));
      i = j;
      if (i == n - 1) rec.push_back("");
    }
  }

  string check4(string const &ip) {
    split(ip, '.');
    if (rec.size() != 4) return "Neither";
    for (const string &s : rec) {
      if (s.size() > 3 || s.empty()) return "Neither";
      if (s.size() > 1 && s[0] == '0') return "Neither";
      for (const char &t : s) {
        if (t < '0' || t > '9') return "Neither";
      }
      int num = stoi(s);
  ...
  string validIPAddress(string queryIP) {
    if (queryIP.find('.') != -1 && queryIP.find(':') != -1) return "Neither";
    int n = queryIP.length();
    if (queryIP.find('.') != -1) return check4(queryIP);
    if (queryIP.find(':') != -1) return check6(queryIP);
    return "Neither";
  }
};